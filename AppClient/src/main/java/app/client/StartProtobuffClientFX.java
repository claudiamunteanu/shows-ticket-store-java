...
public class StartProtobuffClientFX extends Application {
    ...
    private static int defaultAppPort=55556;
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        int serverPort = defaultAppPort;
        try {
            serverPort = Integer.parseInt(clientProps.getProperty("app.server.port"));
        } catch (NumberFormatException ex) {
            System.err.println("Wrong port number " + ex.getMessage());
            System.out.println("Using default port: " + defaultAppPort);
        }
        System.out.println("Using server IP " + serverIP);
        System.out.println("Using server port " + serverPort);
        IAppServices server = new ProtoAppProxy(serverIP, serverPort);
